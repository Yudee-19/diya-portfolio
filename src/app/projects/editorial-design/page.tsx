import Image, { type StaticImageData } from "next/image";
import Link from "next/link";
import { ScrapbookItem } from "@/components/ScrapbookAnimation";
import { Polaroid } from "@/components/Polaroid";
import starIcon from "@/assets/star.svg";
import star2Icon from "@/assets/star-2.svg";
import wriggle from "@/assets/wriggle.svg";
import magazinePhoto from "@/assets/editorial-design/magazine.png";
import layoutDesignPhoto from "@/assets/editorial-design/layout-design.png";
import typographyPhoto from "@/assets/editorial-design/typography.png";

type Direction = "top" | "bottom" | "left" | "right";

interface TopicCard {
  slug: string;
  title: string;
  /** Card photo. */
  photo: StaticImageData;
  /** Scrapbook entrance direction. */
  direction: Direction;
  /** Resting tilt of the card, in degrees. */
  rotation: number;
  /** Entrance stagger. */
  delay: number;
}

const cards: TopicCard[] = [
  { slug: "magazine-design", title: "Magazine Design", photo: magazinePhoto, direction: "left", rotation: -2, delay: 0.1 },
  { slug: "layout-designs", title: "Layout Designs", photo: layoutDesignPhoto, direction: "top", rotation: 1.5, delay: 0.2 },
  { slug: "typography", title: "Typography", photo: typographyPhoto, direction: "right", rotation: -1.5, delay: 0.3 },
];

export default function EditorialDesignPage() {
  return (
    <main className="relative bg-white px-6 py-8 text-black sm:px-10 md:flex md:h-[100svh] md:flex-col md:overflow-hidden md:py-6 lg:px-16 lg:py-8">
      {/* Back link */}
      <Link
        href="/#key-projects"
        className="shrink-0 font-sans text-sm tracking-wide text-gray-500 transition-colors hover:text-black"
      >
        ← Back to projects
      </Link>

      {/* Heading */}
      <div className="mt-5 flex shrink-0 items-start gap-1 lg:mt-3">
        <ScrapbookItem
          direction="left"
          travelDistance={40}
          tiltSteps={3}
          duration={0.6}
          delay={0.05}
        >
          <h1 className="font-playfair text-[clamp(2rem,5vw,4.25rem)] font-bold uppercase leading-none tracking-tight text-[#222220]">
            Editorial Design
          </h1>
        </ScrapbookItem>
        <Image
          src={starIcon}
          alt=""
          unoptimized
          className="mt-1 h-auto w-5 shrink-0 sm:w-7"
        />
      </div>

      {/* Sub-topic cards. From md up the section fills the viewport with no
          scroll, the row centred in the leftover space. */}
      <div className="mt-8 flex justify-center md:mt-0 md:min-h-0 md:flex-1 md:items-center">
        <div className="relative grid w-full max-w-[1100px] grid-cols-1 gap-x-10 gap-y-12 sm:grid-cols-2 md:grid-cols-3">
          {cards.map((c) => (
            <div key={c.slug} className="flex justify-center">
              <ScrapbookItem
                className="w-full max-w-[340px] md:max-w-[16rem] lg:max-w-[19rem] xl:max-w-[21rem]"
                direction={c.direction}
                travelDistance={50}
                tiltSteps={4}
                duration={0.7}
                delay={c.delay}
                maxAngle={12}
                finalRotation={c.rotation}
              >
                <Link
                  href={`/projects/editorial-design/${c.slug}`}
                  className="group block w-full cursor-pointer"
                  aria-label={c.title}
                >
                  <Polaroid
                    className="w-full"
                    photo={c.photo}
                    alt={c.title}
                    caption={c.title}
                  />
                </Link>
              </ScrapbookItem>
            </div>
          ))}

          {/* Decorative scrapbook accents (desktop) */}
          <div className="pointer-events-none absolute inset-0 hidden lg:block">
            <Image src={star2Icon} alt="" unoptimized className="absolute left-[9%] top-[-12%] h-auto w-6" />
            <Image src={starIcon} alt="" unoptimized className="absolute left-[27%] bottom-[-10%] h-auto w-7" />
            <Image src={starIcon} alt="" unoptimized className="absolute left-[63%] top-[-14%] h-auto w-8" />
            <Image src={star2Icon} alt="" unoptimized className="absolute right-[5%] top-[-12%] h-auto w-6" />
            <Image src={wriggle} alt="" unoptimized className="absolute right-[26%] bottom-[-8%] h-auto w-14 -rotate-6 opacity-80" />
          </div>
        </div>
      </div>
    </main>
  );
}
